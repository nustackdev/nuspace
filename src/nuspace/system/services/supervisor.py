"""supervisor: restarts the cells its policy names, with backoff (D13).
...
Backoff: 0.25s, doubled per restart after a failure, capped at 30s, back to
0.25s after an ``ok`` exit.
"""
...
class Policy(CellState):
    """The supervisor's state: ``"<plane>/<cell>"`` -> a policy in :data:`POLICIES`."""
...
def _made() -> nu.Nu:
    """The policy dict written empty if it never was, so a subscription on it resolves.

    Asks the cell's state for the key: a dict never written reads as there.
    """
    listed = Space.planes[PLANE].cells[CELL].state.contains("cells")
    return nu.IfDo(nu.Not(listed), _here(Policy.cells.set(nu.Literal({}))))


def supervise(plane_id: nu.StrArg, cell_id: nu.StrArg, policy: nu.StrArg = ON_FAILURE) -> nu.Nu:
    ...
    """
    cells = Policy.cells
    return atomic(_made() >> _here(cells.set_item(_key(plane_id, cell_id), policy)))


def unsupervise(plane_id: nu.StrArg, cell_id: nu.StrArg) -> nu.Nu:
    """Take a cell off the supervisor. Its live runs are left alone. A no-op when not listed."""
...
def _policy(key: nu.StrAttrRef) -> nu.Nu:
...
def _turn(key: nu.StrAttrRef, plane: nu.StrAttrRef, cell: nu.StrAttrRef) -> nu.Nu:
    """One look: restart after the backoff when due, else wait for ``live`` to move."""
