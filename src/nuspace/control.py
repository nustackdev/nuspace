...
from nuspace.core.shapes import ACTIVE_PAGE
...
def build_primitive_term(op: str, args: dict) -> Nu:
    ...
    ``args`` is a json-safe dict of parameters. ``add_block`` is
    kind-blind: it accepts a raw ``snippet`` (Nu source) and an
    ``app_id``; snippet authoring lives in the caller (see
    ``nuspace.snippets``).
    """
    if op == "add_page":
        slug = args["slug"]
    ...
    if op == "add_block":
        ...
        return AddBlock(slug, snippet, app_id=app_id)

    if op == "update_snippet":
        return UpdateSnippet(args["app_id"], args.get("snippet") or "")
