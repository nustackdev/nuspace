"""The prose surface as one node, and what a Cell holds when it is prose.

:class:`PagesRef` is a component ref like ``ButtonRef`` or ``InputRef``, only
wider: it renders a Plane rail and a Cell canvas instead of a label. Same two
halves as any of them and nothing else, and every one of them is spelled once
in :mod:`nuspace.web.viewers.prose.interactions`. The ref holds no state. It
reads nothing, it remembers nothing and it knows about no store: everything it
ships is handed to it by whoever composed it.

:data:`PROSE` is the other half of what this viewer is. A Cell holds a Nu
...
the same sentence for any viewer that draws Cells.
"""
...
__all__ = ["PROSE", "PagesRef", "prose_source", "starters"]
...
class PagesRef(SpaceRef):
    """Every Plane a tab can open, and one Plane's Cells, as one browser node."""

    _wire_type: ClassVar[str] = "PagesRef"

    @classmethod
    def slot(cls, *, root: type[Space] = Space) -> Self:
        """Mount the surface, seeded with what a new Cell starts life as.
    ...
    def set_tree(self, pages: ListArg[dict]) -> Nu:
        """Replace the rail: every Plane this tab lists, flat."""
        return interactions.set_tree(self, pages)
