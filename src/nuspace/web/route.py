...
:class:`RouteRef` is the browser's own. The route is per view, so the browser
...
:class:`Route` is the server's, and it is a different fact: the browser answers
...
tabs write each other's route.

A route is which Plane is open and nothing else. The browser answers
``{"page_id"}`` and ``page_id`` holds a Plane id: the browser's vocabulary is
what it is, and moving it to the model's words is a deliberate move of its own,
made later.
"""
...
__all__ = ["PLANE_ID", "Route", "RouteRef", "opened", "opens", "per_connection"]


#: The Plane the browser is on, spelled the way the browser spells it.
PLANE_ID = "page_id"


class RouteRef(SpaceRef):
    ...
    the Viewer's op rather than a route one.
    ...
    The name is not ``NavRef``: the ui kit registers one of those, and the last
    registration for a wire type wins, so a bare ``NavRef`` here would replace
    the kit's without saying so.
    """

    _wire_type: ClassVar[str] = "RouteRef"
...
def per_connection(body: nu.Nu) -> nu.With:
    """``body``, with a route record this connection alone can reach.
    ...
    """
    return nu.With(nu.Provide(dict, {}, tag=Route), body=body)


def opened() -> nu.Nu:
    """The Plane this connection has open, or ``""`` when it has none."""
    return nu.If(Route.plane.exists(), nu.ToStr(Route.plane), nu.Str(""))


def opens(plane: nu.StrArg) -> nu.Nu:
    """Remember that this connection opened ``plane``."""
    return Route.plane.set(plane)
