...
def _reads() -> nu.Nu:
    ...
    return nu.Dict.of(
        ...
        home_title=Space.pages["home"].title,
        welcome_snippet=Space.pages["home"].sections["s_welcome"].snippet,
        app_keys=nu.Collect(nu.Iter(Space.apps)),
        page_keys=nu.Collect(nu.Iter(Space.pages)),
        home_section_keys=nu.Collect(nu.Iter(Space.pages["home"].sections)),
    )
