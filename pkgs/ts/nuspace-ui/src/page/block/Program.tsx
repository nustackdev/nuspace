...
// in the gutter (see ./Gutter.tsx), where every block gets them, so what is
// left below is purely what the program produced. On a read-only page even the
// "runs headless" note goes: the page draws output and nothing about it.
//
...
import { useBlockHasUi } from "./address";
import { SourceEditor } from "./Code";

export type ProgramProps = {
	source: string;
	/** Where this block's own refs live in the tree. See ./address.ts. */
	uiPath: Path;
	status: SectionStatus | null;
	/** The page's setting. Off: output only. */
	editable: boolean;
	editing: boolean;
