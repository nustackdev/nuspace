// The ghost input: the one thing on a page that is not a block.
//
...
// block or it is gone the moment it loses focus with nothing in it. The
	...
	return (
		<input
			...
			onKeyDown={(e) => {
				...
				// block does: the page must not also act on a key answered here.
				e.stopPropagation();
				if (open && onKey(e.key)) {
					e.preventDefault();
					return;
