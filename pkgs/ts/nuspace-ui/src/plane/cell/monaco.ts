...
//
// Monaco is ~4MB of the bundle and most plane views never open a code editor,
// so it lives behind a dynamic import and lands in its own chunk. The first
// program cell that flips into code mode pays for it; the document itself
// paints immediately.
//
 ...
 * opened a code box, so flipping the theme on a plane with no open editor does
...
// One subscription for the whole tab, not one per open code box.
