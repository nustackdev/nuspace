...
# A realistic mix: some standing, some one-shot, all touching nu.ui.
REALISTIC = [
    "nu.ui.TextRef(path + '.out').set(nu.Str('hello'))",
    "nu.ForeverDo(nu.Delay(nu.Float(0.05)))",
    "nu.ui.TextRef(path + '.a').set(nu.Str('x')) | nu.ui.TextRef(path + '.b').set(nu.Str('y'))",
    "nu.ForeverDo(nu.Delay(nu.Float(0.02)) >> nu.ui.TextRef(path + '.tick').set(nu.Str('t')))",
]


@pytest.mark.parametrize("warm", [True, False], ids=["warm", "cold"])
async def test_page_open_latency(warm, bounds, settled, capsys):
    host = LoopbackHost()
    ...
    try:
        specs = [SectionSpec(f"s{i}", REALISTIC[i % len(REALISTIC)]) for i in range(SECTIONS)]
